...
impl MediaContentRepo {
    ...
    pub async fn get_scraping_settings(db: &DatabaseConnection) -> Result<Option<serde_json::Value>, AppError> {
        let stmt = Statement::from_sql_and_values(
            DatabaseBackend::Postgres,
            "SELECT id, generate_nfo, download_images, use_chinese_info, \
             movie_image_sources, tv_image_sources, image_language_priority, \
             use_hd_poster, use_hd_backdrop, \
             created_at::text as created_at, updated_at::text as updated_at \
             FROM scraping_settings LIMIT 1",
            [],
        );
        let Some(r) = db.query_one_raw(stmt).await? else {
            return Ok(None);
        };
        Ok(Some(json!({
            "id": col::<Uuid>(&r, "id")?.to_string(),
            "generateNfo": col::<bool>(&r, "generate_nfo").unwrap_or(false),
            "downloadImages": col::<bool>(&r, "download_images").unwrap_or(false),
            "useChineseInfo": col::<bool>(&r, "use_chinese_info").unwrap_or(false),
            "movieImageSources": opt::<serde_json::Value>(&r, "movie_image_sources"),
            "tvImageSources": opt::<serde_json::Value>(&r, "tv_image_sources"),
            "imageLanguagePriority": opt::<serde_json::Value>(&r, "image_language_priority"),
            "useHdPoster": col::<bool>(&r, "use_hd_poster").unwrap_or(false),
            "useHdBackdrop": col::<bool>(&r, "use_hd_backdrop").unwrap_or(false),
            "createdAt": opt::<String>(&r, "created_at"),
            "updatedAt": opt::<String>(&r, "updated_at"),
        })))
    }

    pub async fn update_scraping_settings(
        db: &DatabaseConnection,
        input: serde_json::Value,
    ) -> Result<serde_json::Value, AppError> {
        let existing = Self::get_scraping_settings(db).await?;

        if existing.is_some() {
            let stmt = Statement::from_sql_and_values(
                DatabaseBackend::Postgres,
                "UPDATE scraping_settings SET \
                 generate_nfo = COALESCE($1, generate_nfo), \
                 download_images = COALESCE($2, download_images), \
                 use_chinese_info = COALESCE($3, use_chinese_info), \
                 movie_image_sources = COALESCE($4, movie_image_sources), \
                 tv_image_sources = COALESCE($5, tv_image_sources), \
                 image_language_priority = COALESCE($6, image_language_priority), \
                 use_hd_poster = COALESCE($7, use_hd_poster), \
                 use_hd_backdrop = COALESCE($8, use_hd_backdrop), \
                 updated_at = NOW() \
                 WHERE id = (SELECT id FROM scraping_settings LIMIT 1)",
                [
                    input
                        .get("generateNfo")
                        .and_then(sea_orm::JsonValue::as_bool)
                        .map_or(Value::Bool(None), Value::from),
                    input
                        .get("downloadImages")
                        .and_then(sea_orm::JsonValue::as_bool)
                        .map_or(Value::Bool(None), Value::from),
                    input
                        ...
                        .map_or(Value::Bool(None), Value::from),
                    input
                        .get("movieImageSources")
                        .cloned()
                        .map_or(Value::Json(None), Value::from),
                    input
                        .get("tvImageSources")
                        .cloned()
                        .map_or(Value::Json(None), Value::from),
                    input
                        .get("imageLanguagePriority")
                        .cloned()
                        .map_or(Value::Json(None), Value::from),
                    input
                        .get("useHdPoster")
            ...
            let stmt = Statement::from_sql_and_values(
                DatabaseBackend::Postgres,
                "INSERT INTO scraping_settings (id, generate_nfo, download_images, use_chinese_info, \
                 movie_image_sources, tv_image_sources, image_language_priority, \
                 use_hd_poster, use_hd_backdrop, created_at, updated_at) \
        ...
        Self::get_scraping_settings(db)
            .await?
            .internal("failed to read back settings")
    }
